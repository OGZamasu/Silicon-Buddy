...
class ModelStore(val directory: File) {
    ...
    fun partFile(sha256: String): File = File(directory, "${sha256.lowercase()}.part")

    private fun progressFile(sha256: String): File = File(directory, "${sha256.lowercase()}.part.json")
    ...
    fun deleteEverything(id: String, sha256: String?) {
        ...
        sha256?.let {
            discardPartial(it)
            File(directory, "${it.lowercase()}.gguf").delete()
        }
    }
