...
class PendingInvite(private val saved: SavedStateHandle) {
    ...
    fun offer(invite: PairingInvite?) {
        this.invite = invite
        if (invite == null) saved.remove<String>(KEY) else saved[KEY] = invite.toUriString()
    }

    /**
     * It is being spent. Still asked about on screen while the Mac answers — and after a
     * refusal, so it can be tried again — but no longer saved: whatever the Mac did with the
